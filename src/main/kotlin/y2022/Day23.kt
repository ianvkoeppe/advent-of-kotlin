...
object Day23 {
  ...
  data class Crater(val elves: Set<Point>, val rounds: Int = 0) {
    ...
    fun optimizePlanting(round: Int = -1, directionPriority: Int = 0): Crater {
      ...
      val movements = proposals.mapValues { d -> if (uniqueDestinations.getValue(d.value) == 1) d.value else d.key }.map { it.value }.toSet()

      return when (elves) {
        movements -> copy(rounds = rounds + 1)
        else -> copy(elves = movements, rounds = rounds + 1).optimizePlanting(round - 1, directionPriority = (directionPriority + 1) % directions.size)
      }
    }
    ...
    private fun findProposedMove(elf: Point, directionPriority: Int): Point? {
      return directions.indices.asSequence()
    ...
    fun findEmptyGroundWithinElves(): Int = findYRange().sumOf { y -> findXRange().count { x -> !elves.contains(Point(x, y)) } }
    ...
    private fun findAdjacentSquares(directions: List<Pair<Int, Int>>, point: Point): List<Point> = directions.map { (adjX, adjY) -> Point(point.x + adjX, point.y + adjY) }
  }
  ...
  private fun parse(lines: List<String>): Crater {
    return Crater(lines.indices.flatMap { y -> lines[y].indices.filter { x -> lines[y][x] == '#' }.map { x -> Point(x, y) } }.toSet())
  }
}