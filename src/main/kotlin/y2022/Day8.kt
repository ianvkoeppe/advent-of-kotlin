...
object Day8 {
  ...
  private fun <T>countWithScore(lines: List<String>, visibilityScorer: (Int, List<Int>) -> T): List<List<Visibility<T>>> {
  ...
  private fun <T>score(trees: List<List<Int>>, x: Int, y: Int, visibilityScorer: (Int, List<Int>) -> T, nextIndices: (Int, Int) -> Pair<Int, Int>): T {
    val neighbors = generateSequence(Pair(x, y)) { (x, y) -> nextIndices(x, y) }
  ...
  private fun isValidIndex(trees: List<List<Int>>, x: Int, y: Int): Boolean = x >= 0 && x < trees.size  && y >= 0 && y < trees[x].size
}