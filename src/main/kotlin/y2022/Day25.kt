...
object Day25 {
  ...
  private fun fromSnafu(snafu: String): Long {
    return snafu.map { char -> snafuSymbolValues.getOrElse(char) { char.digitToInt() } }
      .reversed()
  ...
  private fun toSnafuDigit(builder: SnafuBuilder, digit: Int): SnafuBuilder {
    ...
    return builder.copy(snafu = listOf(numberToSymbol.getOrDefault(digitWithCarryOver, digitWithCarryOver.digitToChar())) + builder.snafu, carryOver = carryOver)
  }
}