...
object Day9 {
  ...
  private fun parse(lines: List<String>): List<Movement> = lines.map { it.split(" ") }.map { (distance, amount) -> Movement(distance.first(), amount.toInt()) }
  ...
  private fun moveOnce(rope: KnottedRope, movement: Movement): KnottedRope {
    val movedHead = when (movement.direction) {
