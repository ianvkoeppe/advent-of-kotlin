...
object Day21 {
  data class Expression(val id: String? = null, val value: Long? = null, val operation: Operation? = null) {
    ...
    fun evalOtherSide(otherSide: Long): Long {
      val solved = if (operation == null) otherSide else {
    ...
    fun findUnsolvableExpression(): Expression? = if (operation?.lhs?.canSolve() == true) operation.rhs else operation?.lhs
  }
  data class Operation(val lhs: Expression, val op: String, val rhs: Expression) {
    private val operations: Map<String, (Long, Long) -> Long> = mapOf("+" to Long::plus, "-" to Long::minus, "*" to Long::times, "/" to Long::div)
  ...
  data class Troop(val monkeys: Map<String, Expression>) {
    ...
    private fun resolveExpressions(id: String): Expression {
      ...
      return when {
        ...
        else -> Expression(operation = Operation(resolveExpressions(monkey.operation!!.lhs.id!!), monkey.operation.op, resolveExpressions(monkey.operation.rhs.id!!)))
      }
    }
  }
  ...
  private fun parse(line: String): Pair<String, Expression> = when {
