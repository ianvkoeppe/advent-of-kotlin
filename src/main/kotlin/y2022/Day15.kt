...
object Day15 {
  ...
  data class IdentifiedSignal(val sensor: Point, val beacon: Point, val height: Int = sensor.manhattanDistance(beacon)) {
      ...
      return (sensor.y - height until sensor.y + height).flatMap { y ->
        findRangeOfSignalAt(y)?.let { (start, end) ->
          listOf(Point(start, y), Point(end, y)).flatMap { (x, y) ->
            adjacentSquares.map { (adjX, adjY) -> Point(x + adjX, y + adjY) }
    ...
    fun overlapsWithBeacon(range: Pair<Int, Int>, y: Int): Boolean = range.first <= beacon.x && beacon.x <= range.second && beacon.y == y
  }
  ...
  data class Signals(val signals: List<IdentifiedSignal>) {
    ...
    fun findDistressBeacon(max: Int): Point {
      return signals.asSequence()
        ...
        .filterNot { point -> signals.any { it.isInSignalRange(point) } }.first()
    }
  ...
  private fun parse(lines: List<String>): Signals {
    return Signals(lines.map { line -> line.split(": ") }.map { (sensorLine, beaconLine) ->
