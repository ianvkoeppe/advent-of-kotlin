...
object Day17 {
  ...
  enum class TetrominoType { HORIZONTAL, CROSS, L, VERTICAL, BLOCK }
  ...
  data class Field(val grid: List<Placement> = listOf(), private val points: Set<Point> = grid.flatMap { it.tetromino.points }.toSet(), val width: Int = 7) {
    fun place(tetromino: Tetromino, direction: Int): Field = copy(grid = grid + Placement(tetromino, direction), points = points + tetromino.points)
    fun canPlace(point: Point): Boolean = point.x in (0 until width) && point.y >= 0 && !points.contains(point)
    fun findCycle(): Field? {
      val tetromino = grid.lastOrNull()
      val candidateCycleEnd = grid.dropLast(1).indexOfLast { it.tetromino.type == tetromino?.tetromino?.type && it.direction == tetromino.direction }
      return if (candidateCycleEnd >= 0 && (candidateCycleEnd - (grid.size - candidateCycleEnd) + 1) > 0) {
        val second = Field(grid.subList(candidateCycleEnd + 1, grid.size))
        val first = Field(grid.subList(candidateCycleEnd - second.grid.size + 1, candidateCycleEnd + 1))
    ...
    override fun toString(): String = (height() downTo floor()).joinToString("\n") { y ->
  ...
  data class Tetris(private val directions: List<Char>, private val field: Field = Field(), private val d: Int = 0, private val t: Int = 0) {
    ...
    private val vertical = Tetromino(TetrominoType.VERTICAL, setOf(Point(0, 0), Point(0, 1), Point(0, 2), Point(0, 3)))
    ...
    private fun settle(tetromino: Tetromino): Tetris {
      ...
      return when {
        ...
        else -> copy(field = field.place(horizontal, d), d = nextDirection, t = (t + 1) % tetrominoes.size)
      }
    }
    private fun moveHorizontalIfPossible(tetromino: Tetromino): Tetromino = when {
