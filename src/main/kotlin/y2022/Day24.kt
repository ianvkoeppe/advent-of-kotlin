...
object Day24 {
  ...
  data class Valley(val tiles: Map<Position, Set<Char>>, val minutes: Int = 0, val width: Int = tiles.maxOf { it.key.x }, val height: Int = tiles.maxOf { it.key.y }) {
    ...
    private val movements: Map<Char, (Position, Int, Int) -> Position> = mapOf(
    ...
    private fun blizzard(): Valley = copy(tiles = groupByPosition(tiles.flatMap { (position, blizzards) ->
      blizzards.map { blizzard -> if (blizzard in movements) blizzard(position, blizzard) to blizzard else position to blizzard }
    }), minutes = minutes + 1)
    private fun blizzard(position: Position, blizzard: Char): Position = movements.getValue(blizzard)(position, width, height)

    private fun findPossibleMoves(position: Position): Set<Position> {
      return adjacentSquares.map { (adjX, adjY) -> Position(position.x + adjX, position.y + adjY) }
        .filter { (x, y) -> x in 1 until width && y in 0..height }
        .toSet()
    }

    private fun isSafe(position: Position): Boolean = tiles[position]?.all { it == '.' } ?: true
    private fun bound(index: Int, min: Int, max: Int): Int = (max - min + 1).let { range -> ((index - min) % range + range) % range + min }
  }
  ...
  private fun parse(lines: List<String>): Valley {
    return Valley(groupByPosition(lines.flatMapIndexed { y, row -> row.mapIndexed { x, blizzard -> Position(x, y) to blizzard } }))
  }
  private fun groupByPosition(blizzards: List<Pair<Position, Char>>): Map<Position, Set<Char>> = blizzards
    .groupBy { it.first }
