...
object Day12 {
  ...
  private fun findCoordinatesOf(hill: List<List<Char>>, targets: Set<Char>): List<Pair<Int, Int>> {
    return hill.flatMapIndexed { x, chars ->
      chars.mapIndexed { y, c -> if (c in targets) x to y else null }.filterNotNull()
    }
  }
  ...
  private fun isValidPosition(
    ...
    return currentHeight != null &&
      nextHeight != null &&
      (abs(currentHeight - nextHeight) <= 1 || nextHeight > currentHeight)
  }
