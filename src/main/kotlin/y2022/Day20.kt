...
object Day20 {
  data class Node<T>(val v: T, var prev: Node<T>? = null, var next: Node<T>? = null) {
    fun move(times: Int, node: Node<T> = this) = if (times >= 0) moveTowardsTail(node, times) else moveTowardsHead(node, -times)
  ...
  data class EncryptedNumbers(private val head: Node<Long>, val original: List<Node<Long>>) {
    companion object Factory {
      fun from(values: List<Long>): EncryptedNumbers {
        ...
        val linked = nodes.zipWithNext { current, next -> current.next = next; next.prev = current; current } + nodes.last()
    ...
    fun shuffle() {
      original.forEach { num ->
        val distance = generateSequence(num.v) { v -> (v % original.size).toInt() + (v / original.size) }.first { abs(it) < original.size }.toInt()
        num.move(distance)
      }
    }

    fun unlinkAt(v: Long, node: Node<Long>? = head): List<Long> = if (node?.v == v) generateSequence(unlinkAt(node)) { it.next }.map { it.v }.toList() else unlinkAt(v, node?.next)
  ...
  private fun parseWithKey(lines: List<String>, key: Long = 1): EncryptedNumbers = EncryptedNumbers.from(lines.map(String::toLong).map { it * key })
}