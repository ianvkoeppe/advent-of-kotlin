...
object Day19 {
  enum class Resource { GEODE, OBSIDIAN, CLAY, ORE  }
  data class Blueprint(val costs: Map<Resource, Map<Resource, Int>>, val robots: Map<Resource, Int> = mapOf(Resource.ORE to 1), val resources: Map<Resource, Int> = mapOf()) {
    ...
    private fun mineResources(turns: Int = 1): Blueprint = copy(resources = robots.map { (robot, count) -> robot to resources.getOrDefault(robot, 0) + (turns * count) }.toMap())
    private fun findTimeToBuildRobots(): Map<Resource, Int> = Resource.values()
      .filter(::shouldBuild)
    ...
    private fun findTimeToBuildRobot(robot: Resource): Int? {
      ...
      return max(0, costs.maxOf { (type, cost) -> ceil((cost.toDouble() - resources.getOrDefault(type, 0)) / robots.getValue(type)) }.toInt()) + 1
    }
    private fun shouldBuild(robot: Resource): Boolean = robot == Resource.GEODE || robots.getOrDefault(robot, 0) < costs.values.maxOf { it.getOrDefault(robot, 0) }
    ...
    private fun findMaxPossibleRemainingGeodes(minutes: Int): Int = mineResources(minutes).numberOfGeodes() + (minutes * (minutes + 1) / 2)
    private fun numberOfGeodes(): Int = resources[Resource.GEODE] ?: 0
  }

  fun partOne(lines: List<String>): Int = parse(lines).map(Blueprint::findMaxCrackableGeodes).mapIndexed { i, max -> (i + 1) * max }.sum()
  ...
  private fun parseBlueprint(line: String): Blueprint {
    return Blueprint(blueprint.findAll(line).map { match ->
      val (robotType, firstAmount, firstType) = match.groupValues.drop(1)
      val (secondAmount, secondType) = match.groupValues.drop(5)
      val costs = listOf(firstType to firstAmount, secondType to secondAmount).filterNot { it.first == "" }.associate { (type, amount) -> Resource.valueOf(type.uppercase()) to amount.toInt() }
