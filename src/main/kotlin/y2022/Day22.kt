...
object Day22 {
  ...
  data class LineSegment(val start: Point, val end: Point) {
    ...
    fun contains(point: Point): Boolean = start.x <= point.x && point.x <= end.x && start.y <= point.y && point.y <= end.y
    fun points(): List<Point> = generateSequence(start) { p -> if (isHorizontal()) p.copy(x = p.x + 1) else p.copy(y = p.y + 1) }.takeWhile { it != end }.toList() + end
    fun pointsBeyondSegment(): Set<Point> = if (isHorizontal()) setOf(start.copy(x = start.x - 1), end.copy(x = end.x + 1)) else setOf(start.copy(y = start.y - 1), end.copy(y = end.y + 1))
    fun isIncreasingOrDecreasingManhattanDistance(other: LineSegment): Boolean =
      isIncreasingManhattanDistance(points(), other.points()) || isIncreasingManhattanDistance(points().reversed(), other.points().reversed())
  ...
  enum class Direction { EAST, SOUTH, WEST, NORTH;
    ...
    fun turnNinetyDegrees(clockwise: Int): Direction = Direction.values()[(ordinal + clockwise + Direction.values().size) % Direction.values().size]
  }
  ...
  data class Board(val points: Map<Point, Char>, val sizeOfEdge: Int) {
    ...
    fun findStartPosition(): Position = Position(Point(points.keys.filter { it.y == 0 }.minOf { it.x }, 0), Direction.EAST)

    fun toMapLinkingOppositeEdge(): BoardMap {
      val edges = findEdges()
      val links = findEdges().flatMap { edge ->
    ...
    private fun isOppositeEdge(f: LineSegment, s: LineSegment): Boolean = when {
    ...
    fun toMapLinkingAs3DCube(linked: Map<LineSegment, LineSegment> = mapOf()): BoardMap {
      ...
      return when (linked.size) {
        ...
        edges.size - 2 -> {
          ...
          val innerCorners = edges
            .flatMap(LineSegment::pointsBeyondSegment)
            .filter { point -> findNeighbors(point).count { n -> points.contains(n) || edges.any { linked[it]?.contains(point) == true } } == 4 }.toSet()
          val toPair = innerCorners.map { corner ->
            edges.filter { edge ->
              val overlaps = edges.filterNot { it == edge }.firstOrNull { it.points().any(edge::contains) }
              findNeighbors(corner).any { n -> edge.contains(n) || (linked[overlaps]?.points() ?: setOf()).contains(n) }
    ...
    private fun pairedEdgesToPoints(edges: Map<LineSegment, LineSegment>): BoardMap {
      val links = edges.flatMap { (f, s) ->
        val direction = findExitDirection(f)
        val newDirection = findExitDirection(s).turnNinetyDegrees(2)
        val firstOrder = if (direction == newDirection || f.isIncreasingOrDecreasingManhattanDistance(s)) f.points() else f.points().reversed()
        firstOrder.zip(s.points()).map { (f, s) -> Position(f, direction) to Position(s, newDirection) }
      }.toMap()

      return BoardMap(points, links)
    }
    private fun findExitDirection(edge: LineSegment): Direction = when {
    ...
    private fun findEdgesPoints(): Set<Point> = points.keys.filter { point -> findNeighbors(point).count(points::contains) < 4 }.toSet()
    private fun findNeighbors(point: Point): Set<Point> = adjacentSquares.map { (adjX, adjY) -> Point(point.x + adjX, point.y + adjY) }.toSet()
    private fun findSegmentContainingPoint(edgePoints: Set<Point>, point: Point): Set<LineSegment> =
      findEdgeSegmentsFromPoints(edgePoints.filter { p -> p.x == point.x }) + findEdgeSegmentsFromPoints(edgePoints.filter { p -> p.y == point.y })
    private fun findEdgeSegmentsFromPoints(points: List<Point>): Set<LineSegment> {
      val sorted = points.sortedWith(compareBy({ it.x }, { it.y }))
      val (groups, lastGroup) = sorted.drop(1).fold(Pair<Set<Set<Point>>, Set<Point>>(setOf(), setOf(sorted.first()))) { (groups, group), point ->
        if (group.last().x + 1 == point.x || group.last().y + 1 == point.y) groups to group + point else (groups + setOf(group)) to setOf(point)
      }
      return (groups + setOf(lastGroup)).flatMap { group -> group.chunked(sizeOfEdge).filter { it.size == sizeOfEdge }.map { segment -> LineSegment(segment.first(), segment.last()) } }.toSet()
    }
  }
  ...
  private fun parseBoard(lines: List<String>): Board {
    val points = lines.indices.flatMap { y -> lines[y].indices.filterNot { x -> lines[y][x] == ' ' }.map { x -> Point(x, y) to lines[y][x] } }.toMap()
  ...
  private fun parseMovements(line: String): List<Movement> {
    val ms = line.toList().fold(Pair<List<Movement>, String>(listOf(), "")) { movements, char ->
