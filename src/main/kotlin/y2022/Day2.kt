...
object Day2 {
  enum class Choice(val score: Int) { ROCK(1), PAPER(2), SCISSORS(3) }
  ...
  private val OUTCOMES = mapOf(-1 to 0, 0 to 3, 1 to 6) // Take advantage of the idea that choice at index beats choice at index - 1, draws at same index, and loses to index + 1.
  ...
  private fun score(theirMove: Choice, yourMove: Choice): Int {
    println("$theirMove | $yourMove | ${theirMove.ordinal - yourMove.ordinal}")
    return yourMove.score + OUTCOMES.map { (indexOffset, score) ->
