...
object Day21 {
  ...
  private fun countUniverses(players: List<Player>, winningScore: Int, state: State = State(players.associate { it.n to it.initialPosition },  mapOf(), 0)): Map<Int, Long> {
    ...
    return diracRolls
      .map { (roll, count) -> countUniverses(players.drop(1) + players.first(), winningScore, turn(players.first(), state, roll)).mapValues { (_, v) -> count * v } }
  ...
  private fun turn(player: Player, state: State, roll: Int): State {
    ...
    return state.copy(positions = state.positions + (player.n to position), scores = updatedScore, turn = state.turn + 1)
  }
}