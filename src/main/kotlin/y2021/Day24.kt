...
object Day24 {
  private data class Val(val id: String? = null, val primitive: Int? = null) {
    override fun toString(): String = id ?: primitive.toString()
  }

  private data class Op(val left: Expr, val op: String, val right: Expr?) {
  ...
  private data class Expr(val value: Val? = null, val op: Op? = null) {
    override fun toString(): String = value?.toString() ?: op.toString()
  }

  private data class Assignment(val variable: String, val op: Op) {
    override fun toString(): String = "$variable = $op"
  }
  ...
  private fun solveBackwards(
    ...
    if (digitPosition < 0)
      return findNumberEqualingZByComparator(targetZToDigit, { -it.first }).toLong() to
        findNumberEqualingZByComparator(targetZToDigit, { it.first }).toLong()
  ...
  private fun findCandidateDigits(
    ...
    return (1..9)
      ...
      .map { candidateDigit ->
        targetZValues
          .flatMap { targetZValue ->
            (0 until constants.first)
              .flatMap { A ->
                listOfNotNull(
  ...
  private fun secondValidCheck(candidateDigit: Int, targetZValue: Int, constants: Triple<Int, Int, Int>, A: Int): Int? {
    ...
    return if (
      position % 26 + constants.second != candidateDigit &&
        position / constants.first * 26 + candidateDigit + constants.third == targetZValue
    )
      position
    else null
  }

  private fun findNumberEqualingZByComparator(
  ...
  private fun optimize(expr: Expr): Expr {
    return when (expr.op?.op) {
      "add" ->
        if (expr.op.left.value?.primitive == 0) expr.op.right!!
        else if (expr.op.right?.value?.primitive == 0) expr.op.left else expr
      "mul" -> {
        if (expr.op.left.value?.primitive == 1) expr.op.right!!
        else if (expr.op.right?.value?.primitive == 1) expr.op.left
        else if (expr.op.left.value?.primitive == 0 || expr.op.right?.value?.primitive == 0)
          Expr(value = Val(primitive = 0))
        else expr
      }
