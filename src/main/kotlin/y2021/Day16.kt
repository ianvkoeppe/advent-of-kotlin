...
object Day16 {
  ...
  private val hexToBinary = hexes.mapIndexed { i, char -> char to String.format("%4s", i.toString(2)).replace(' ', '0') }.toMap()
  ...
  private fun parse(binary: String): Packet {
    ...
    return when ((type to lengthType)) {
      ...
      (type to 0) -> createOperatorPacket(version, type, binary, packetBitLength) { packets -> packets.sumOf { packet -> packet.binary.length } }
