...
object Day14 {
  ...
  private fun findMaxMinDifferenceOfElementFrequenciesAfterSteps(
    ...
    val counts =
      step(elementPairs, rules, steps)
        .flatMap { (element, count) -> listOf(element[0] to count, element[1] to count) }
        .fold(mapOf<Char, Long>()) { counts, (k, v) ->
          counts + (k to counts.getOrDefault(k, 0) + v)
        }
        .mapValues { (_, v) -> ceil(v / 2.0).toLong() }
        .toMap()
    ...
    val next =
      polymer
        .flatMap { (element, count) ->
          listOf(
            element[0] + rules.getValue(element) to count,
            rules.getValue(element) + element[1] to count
          )
        }
        .fold(mapOf<String, Long>()) { counts, (k, v) ->
          counts + (k to counts.getOrDefault(k, 0) + v)
        }
