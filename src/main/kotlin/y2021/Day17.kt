...
object Day17 {
  ...
  private fun parse(lines: List<String>): Pair<IntRange, IntRange> {
    val (xRange, yRange) = lines.first().substringAfter(':').split(",").map(String::trim)
      .map {
  ...
  private fun findVelocitiesThatHitTarget(xRange: IntRange, yRange: IntRange): Set<Pair<Int, Int>> {
    return (1..xRange.last).flatMap { xVelocity ->
      (yRange.first - 1..abs(yRange.first + 1)).filter { yVelocity ->
        hitsTarget(0, 0, xVelocity, yVelocity, xRange, yRange)
