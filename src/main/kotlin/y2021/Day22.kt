...
object Day22 {
  ...
  fun partOne(lines: List<String>): Long {
    ...
    return run(instructions).sumOf { area -> (area.x.last - area.x.first + 1) * (area.y.last - area.y.first + 1) * (area.z.last - area.z.first + 1) }
  }
  ...
  fun partTwo(lines: List<String>): Long {
    return run(parse(lines)).sumOf { area -> (area.x.last - area.x.first + 1) * (area.y.last - area.y.first + 1) * (area.z.last - area.z.first + 1) }
  }
  ...
  private fun turn(current: List<Area3D>, i: Instruction): List<Area3D> {
    val sliced = current.flatMap { area ->
  ...
  private fun intersect(first: LongRange, second: LongRange): LongRange = max(first.first, second.first)..min(first.last, second.last)
