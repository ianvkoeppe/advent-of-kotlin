...
object Day4 {
  fun partOne(lines: List<String>): Long {
    ...
    val (winningRound, board) = boards.map { board -> findMinWinRoundBy(board, nums) to board }.minByOrNull { it.first }!!
    return calculateBoardScore(board, nums, winningRound)
  }

  fun partTwo(lines: List<String>): Long {
    val (nums, boards) = parse(lines)
    val (winningRound, board) = boards.map { board -> findMinWinRoundBy(board, nums) to board }.maxByOrNull { it.first }!!
    return calculateBoardScore(board, nums, winningRound)
  }
  ...
  private fun findMinWinRoundBy(board: Board, nums: LinkedHashSet<Long>): Int {
    val minRowWin = board.indices.map { checkWin(board, nums, it to 0) { (x, y) -> x to y + 1 } }.filterNotNull().minOrNull()
    val minColWin = board.indices.map { checkWin(board, nums, 0 to it) { (x, y) -> x + 1 to y } }.filterNotNull().minOrNull()
    return listOfNotNull(minRowWin, minColWin).minOrNull()!!
  }

  private fun checkWin(board: Board, nums: LinkedHashSet<Long>, start: Pair<Int, Int> = (0 to 0), inc: (Pair<Int, Int>) -> Pair<Int, Int>): Int? {
    val hitRounds = generateSequence(start, inc)
