...
object Day19 {
  ...
  private val xRotations =
    generateSequence(identity) { next -> { p -> xRotation(next(p)) } }.take(4).toList()
  ...
  private val zRotations =
    generateSequence(identity) { next -> { p -> zRotation(next(p)) } }.take(4).toList()
  private val rotations: List<(Point3D) -> Point3D> =
    (xRotations + yRotations).flatMap { orient ->
      zRotations.map { rotate -> { point -> rotate(orient(point)) } }
    }
        ...
        for (s in transposed) {
          ...
          if (overlapPairs.size >= 12) {
            ...
            return LocatedScanner(
              second.number,
              transposed
                .map { b -> Point3D(b.x + position.x, b.y + position.y, b.z + position.z) }
                .toSet(),
              position
            )
          }
  ...
  private fun findDistanceRelativeToPoint(
    points: Set<Point3D>,
    point: Point3D
  ): Map<Point3D, Point3D> {
