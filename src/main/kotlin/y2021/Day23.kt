...
object Day23 {
  data class Burrow(val hallway: List<Char>, val rooms: Map<Char, List<Char>>, val cost: Int = 0) {
    ...
    private val allowedStoppingSquares =
      (0..10).filterNot { roomEntrancePositions.containsValue(it) }

    fun isOrganized(): Boolean = rooms.all { (k, v) -> v.all { it == k } }

    fun findCandidateMoves(): List<Burrow> {
      val candidateMovesToRooms =
        hallway
          .withIndex()
          .filter { (_, amphipod) -> costMagnitudes.containsKey(amphipod) }
          .mapNotNull { (index, _) -> canMoveToRoom(index) }
      return candidateMovesToRooms.ifEmpty {
        roomEntrancePositions.flatMap { (room) -> canMoveOutOfRoom(room) }
      }
    }
    ...
    private fun canMoveToRoom(positionOfAmphipodInHallway: Int): Burrow? {
      ...
      val noObstructions =
        canMoveToHallway(positionOfAmphipodInHallway, roomEntrancePositions.getValue(amphipod))
      return if (roomIsReady && noObstructions)
        newBurrow(positionOfAmphipodInHallway, ' ', amphipod, amphipod)
      else null
    }
    ...
    private fun canMoveOutOfRoom(roomType: Char): List<Burrow> {
      ...
      return allowedStoppingSquares
        .filter { canMoveToHallway(roomEntrancePositions.getValue(roomType), it) }
        .map { square ->
          newBurrow(square, rooms.getValue(roomType).first { it != ' ' }, roomType, ' ')
        }
    }

    private fun canMoveToHallway(start: Int, destination: Int): Boolean {
      val adjacent =
        if (start < destination) start + 1 else start - 1 // Exclude square housing the amphipod
      return (min(adjacent, destination)..max(adjacent, destination)).all { square ->
        hallway[square] == ' '
      }
    }

    private fun newBurrow(
      hallwayIndex: Int,
      hallwayValue: Char,
      roomType: Char,
      roomValue: Char
      ...
      val c =
        abs(hallwayIndex - roomEntrancePositions.getValue(roomType)) + r.size -
          r.count { it != ' ' } + if (roomValue == ' ') 0 else 1
      return this.copy(
        hallway = h,
    ...
    private fun newRoom(roomType: Char, roomValue: Char): List<Char> {
      val prefix =
        if (roomValue == ' ') rooms.getValue(roomType).takeWhile { it == ' ' }
        else rooms.getValue(roomType).takeWhile { it == ' ' }.drop(1)
  ...
  fun partTwo(lines: List<String>): Int {
    ...
    val expanded =
      burrow.copy(
        rooms =
          burrow.rooms.mapValues { (k, v) -> listOf(v.first()) + insert.getValue(k) + v.last() }
      )
  ...
  private fun parse(lines: List<String>): Burrow {
    ...
    return Burrow(
      List(11) { ' ' },
      mapOf('A' to rooms[0], 'B' to rooms[1], 'C' to rooms[2], 'D' to rooms[3])
    )
  }
