...
object Day23 {
  private data class Burrow(val hallway: List<Char>, val rooms: Map<Char, List<Char>>, val cost: Int = 0) {
    ...
    private fun canMoveToRoom(positionOfAmphipodInHallway: Int): Burrow? {
      ...
      return if (roomIsReady && noObstructions) newBurrow(positionOfAmphipodInHallway, ' ', amphipod, amphipod)
      else null
    }
    ...
    private fun newBurrow(hallwayIndex: Int, hallwayValue: Char, roomType: Char, roomValue: Char): Burrow {
      ...
      val c =
        abs(hallwayIndex - roomEntrancePositions.getValue(roomType)) + r.size - r.count { it != ' ' } +
          if (roomValue == ' ') 0 else 1
      return this.copy(
        hallway = h,
  ...
  fun partTwo(lines: List<String>): Int {
    ...
    val insert =
      mapOf('A' to listOf('D', 'D'), 'B' to listOf('C', 'B'), 'C' to listOf('B', 'A'), 'D' to listOf('A', 'C'))
    val expanded =
      burrow.copy(rooms = burrow.rooms.mapValues { (k, v) -> listOf(v.first()) + insert.getValue(k) + v.last() })
