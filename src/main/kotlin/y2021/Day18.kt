...
object Day18 {
  ...
  internal fun flatten(fishNum: JsonArray, depth: Int = 0): List<Pair<Int, Int>> {
    val left =
      if (fishNum.first().isJsonPrimitive) listOf(fishNum.first().asInt to depth)
      else flatten(fishNum.first().asJsonArray, depth + 1)
    val right =
      if (fishNum.last().isJsonPrimitive) listOf(fishNum.last().asInt to depth)
      else flatten(fishNum.last().asJsonArray, depth + 1)
    return left + right
  }
